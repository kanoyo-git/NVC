...
class NVC:
    def __init__(
        ...
        try:
            ...
            if last_nvc is None or last_nvc.pth_path != self.pth_path:
                set_synthesizer()
            else:
                self.tgt_sr = last_nvc.tgt_sr
                self.if_f0 = last_nvc.if_f0
                self.version = last_nvc.version
                self.is_half = last_nvc.is_half
                self.net_g = last_nvc.net_g

            if last_nvc is not None and hasattr(last_nvc, "model_rmvpe"):
                self.model_rmvpe = last_nvc.model_rmvpe
            if last_nvc is not None and hasattr(last_nvc, "model_fcpe"):
                self.model_fcpe = last_nvc.model_fcpe
        except:
