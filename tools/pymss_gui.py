    ...
    def __init__(
        ...
        try:
            ...
            self._save_pool = ThreadPoolExecutor(
                max_workers=2,
                thread_name_prefix="nvc-msst-save",
            )
        except Exception:
            self.close()
            raise
...
def _pymss_worker_main(request_path):
    ...
    try:
        ...
        worker_logger = logging.getLogger("nvc.pymss.worker")
...
def _pymss_worker_events(
    ...
    worker_code = (
        ...
        "sys.argv[:]=[sys.argv[0]];"
        "from tools.pymss_gui import _pymss_worker_main;"
    ...
    with tempfile.TemporaryDirectory(prefix="nvc-pymss-") as temp_root:
