...
GUI_STATIC_DIR = Path(__file__).with_name("gui_static")
...
def _missing_assets_response():
    ...
    return JSONResponse(
        ...
        content={
            "error": {
                "message": "GUI static assets are not built. Build the GUI according to https://github.com/pymss-project/pymss/blob/main/README.md, then start the server again.",
                "type": "server_error",
                "param": None,
                "code": "gui_assets_missing",
            }
        },
    )
...
def register_gui_routes(app, static_dir=None):
    """Register GUI static-file and SPA fallback routes.
    ...
    Returns:
        Any: Computed result."""
    static_dir = Path(static_dir) if static_dir is not None else GUI_STATIC_DIR

    @app.get("/ui")
    async def redirect_gui_root():
        """Implement the redirect gui root helper.
    ...
    @app.get("/ui/")
    async def gui_index():
        """Implement the gui index helper.
    ...
    async def gui_asset(asset_path: str):
        """Implement the gui asset helper.
    ...
    async def gui_spa_fallback(_spa_path: str):
        """Implement the gui spa fallback helper.
