...
- 現在最も強力なオープンソースの人間の声のピッチ抽出モデル RMVPE をトレーニングし、NVC のトレーニング、オフライン/リアルタイム推論に使用。pytorch/onnx/DirectML をサポート
...
- ~~crepe の精度サポートと NVC-config の同期：既に RMVPE に置き換えられているため不要。これをサポートするには torchcrepe ライブラリも同期する必要があり、面倒~~
- F0 エディタとの連携
...
- タブページに「よくある質問」を追加（または github-nvc-wiki を参照）
...
- GUI がシステムの地域に基づいて言語を変更する機能をサポート（現在サポートされているのは en_US、ja_JP、zh_CN、zh_HK、zh_SG、zh_TW、サポートされていない場合はデフォルトで en_US になります）
