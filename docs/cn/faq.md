...
## Q3:训练结束推理没看到训练集的音色
点刷新音色再看看，如果还没有看看训练有没有报错，控制台和gui的截图，logs/实验名下的log，都可以发给开发者看看。<br>

## Q4:如何分享模型
  nvc_root/logs/实验名 下面存储的pth不是用来分享模型用来推理的，而是为了存储实验状态供复现，以及继续训练用的。用来分享的模型应该是weights文件夹下大小为60+MB的pth文件；<br>
...
## Q6:GUI弹出Expecting value: line 1 column 1 (char 0).
请关闭系统局域网代理/全局代理。<br>
...
## Q7:不用GUI如何通过命令训练推理
训练脚本：<br>
可先跑通GUI，消息窗内会显示数据集处理和训练用命令行；<br>
...
## Q13:如何中断和继续训练
现阶段只能关闭GUI控制台双击go-gui.bat重启程序。网页参数也要刷新重新填写；<br>
...
win平台会报这个错，装上https://aka.ms/vs/17/release/vc_redist.x64.exe这个再重启GUI就好了。
